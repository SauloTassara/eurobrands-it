...
export async function initNarrative(controller,{reducedMotion=false}={}){
  ...
  const updateOriginVisuals=progress=>{
    const portalProgress=clamp(progress/.52),paperProgress=clamp(progress/.52),reveal=clamp((progress-.60)/.18);
    setPortalProgress(portalProgress);root.style.setProperty('--paper-progress',paperProgress.toFixed(4));root.style.setProperty('--origin-reveal',reveal.toFixed(4));root.style.setProperty('--origin-clip',`${((1-reveal)*12).toFixed(3)}%`);
    body.classList.toggle('origin-paper-covered',progress>=.46&&progress<.995);body.classList.toggle('origin-content-ready',progress>=.595);
  };
  const activate=name=>{
  ...
  activate('hero');fitOriginWords();updatePortalCoverScale();

  const hasNarrative=Boolean(window.gsap&&window.ScrollTrigger);
  if(reducedMotion||!hasNarrative){
    body.dataset.storyMode=reducedMotion?'reduced-motion':'static-fallback';body.classList.remove('origin-paper-covered');body.classList.add('reduced-story','origin-content-ready');root.style.setProperty('--origin-reveal','1');root.style.setProperty('--origin-clip','0%');controller?.setNarrativeState?.('hero');
    ...
    const refresh=()=>{fitOriginWords();updatePortalCoverScale();};
    const onResize=()=>refresh(),onOrientation=()=>refresh(),onPageShow=event=>{if(event.persisted)refresh();};
    ...
    return{refresh,dispose(){observer.disconnect();removeEventListener('resize',onResize);removeEventListener('orientationchange',onOrientation);removeEventListener('pageshow',onPageShow);}};
  }
  ...
  const build=({mobile=false}={})=>{
    ...
    scenes.forEach(section=>{
      ...
      const timeline=gsap.timeline({scrollTrigger:{trigger:section,start:name==='hero'?'top top':'top bottom',end:name==='origin'||name==='contact'?'bottom bottom':'bottom top',scrub:.82,invalidateOnRefresh:true,onEnter:()=>activate(name),onEnterBack:()=>activate(name),onUpdate:self=>{
        ...
        if(name==='origin')updateOriginVisuals(p);
        if(name==='bridge'){root.style.setProperty('--route-progress',p.toFixed(4));const items=[...section.querySelectorAll('.territory')],active=Math.min(items.length-1,Math.floor(p*items.length));items.forEach((item,i)=>item.classList.toggle('is-active',i===active));}
        if(name==='motion')root.style.setProperty('--copper-progress',Math.min(1,p*1.65).toFixed(4));
        if(name==='contact'){const portalProgress=Math.max(0,1-p*2.4);root.style.setProperty('--copper-progress',Math.max(0,1-p*1.8).toFixed(4));root.style.setProperty('--paper-progress',portalProgress.toFixed(4));setPortalProgress(portalProgress);root.style.setProperty('--route-progress',Math.max(0,1-p*2.2).toFixed(4));}
      }}});
  ...
  const refresh=()=>{clearTimeout(refreshTimer);refreshTimer=setTimeout(()=>{fitOriginWords();updatePortalCoverScale();controller?.resize?.();requestAnimationFrame(()=>{ScrollTrigger.refresh();updateOverall();});},180);};
  ...
  addEventListener('resize',onResize,{passive:true});addEventListener('orientationchange',onOrientation,{passive:true});addEventListener('pageshow',onPageShow);
  fitOriginWords();updatePortalCoverScale();controller?.resize?.();ScrollTrigger.refresh();
